/**
 * AsciiDoc-aware spell-checking via nspell/native browser modes.
 *
...
export type SpellcheckMode = "nspell" | "native";

let activeSpellcheckMode: SpellcheckMode = "nspell";
...
const nspellSpellcheckPlugin = ViewPlugin.fromClass(
  class {
...
const nspellSpellcheckContextMenu = EditorView.domEventHandlers({
...
const nativeSpellcheckExtension = EditorView.contentAttributes.of({ spellcheck: "true" });

// =====================================================
// Exports
// =====================================================

export function setSpellcheckMode(mode: SpellcheckMode): void {
  activeSpellcheckMode = mode;
}

/** Resolve the requested mode to the mode that can actually run in this webview. */
export function resolveSpellcheckMode(mode: SpellcheckMode = activeSpellcheckMode): SpellcheckMode {
  if (mode === "nspell" && !spell) return "native";
  return mode;
}

/** Returns the CM6 extension array for spell-checking. */
export function spellcheckExtension(mode: SpellcheckMode = activeSpellcheckMode) {
  activeSpellcheckMode = mode;
  if (resolveSpellcheckMode(mode) === "native") return [nativeSpellcheckExtension];
  return [nspellSpellcheckPlugin, nspellSpellcheckContextMenu];
}
...
/** Returns whether the active spell-check mode is ready. */
export function isSpellcheckReady(): boolean {
  return resolveSpellcheckMode(activeSpellcheckMode) === "native" || spell !== null;
}
