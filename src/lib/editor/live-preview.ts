...
interface VerseBlockInfo {
  type: "verse";
  attrLine: number;   // line with [verse,...]
  openLine: number;   // line with opening ____ or first paragraph line
  closeLine: number;  // line with closing ____ or last paragraph line
  author: string;
  citetitle: string;
}
...
interface VideoPreviewBlockInfo {
  type: "video";
  titleLine: number; // line with .Caption text, or -1 if none
  macroLine: number; // line with video::...
  info: ParsedVideoMacro;
}

interface AudioPreviewBlockInfo {
  type: "audio";
  titleLine: number; // line with .Caption text, or -1 if none
  macroLine: number; // line with audio::...
  info: ParsedAudioMacro;
}
...
// ── Raw line height measurement probe ─────────────────────────────────────────
// A hidden off-screen element that mirrors a .cm-line's CSS context so we can
// synchronously measure what the raw text height will be before the dispatch
// that transitions a line from preview -> raw mode.
let _rawMeasureProbe: HTMLDivElement | null = null;
let _rawMeasureProbeWidth = 0;
...
/**
 * Returns (creating if needed) a hidden div that lives inside the editor DOM
 * so it inherits the same font, line-height, and white-space as a real .cm-line.
 * Width is kept in sync with the editor's content area so that line-wrapping
 * behaviour matches exactly.
 *
 * Safe to call only from user event handlers (mouseup) — never from inside a
 * CM6 update cycle, because getBoundingClientRect() forces a synchronous layout.
 */
function getRawMeasureProbe(view: EditorView): HTMLDivElement {
  if (_rawMeasureProbe && !_rawMeasureProbe.isConnected) {
    _rawMeasureProbe = null;
    _rawMeasureProbeWidth = 0;
  }

  if (!_rawMeasureProbe) {
    _rawMeasureProbe = document.createElement("div");
    _rawMeasureProbe.className = "cm-line";
    _rawMeasureProbe.setAttribute("aria-hidden", "true");
    _rawMeasureProbe.style.cssText
      = "position:fixed;top:-9999px;left:-9999px;visibility:hidden;"
      + "pointer-events:none;white-space:pre-wrap;word-break:break-all;overflow:hidden;"
      + "padding-left:0 !important;padding-right:0 !important;"
      + "font-family:monospace;line-height:1.6;";
    view.dom.appendChild(_rawMeasureProbe);
  }
  ...
  if (sampleLine) {
    ...
    textAreaWidth = sampleLine.getBoundingClientRect().width
      - (parseFloat(cs.paddingLeft) || 0)
      - (parseFloat(cs.paddingRight) || 0);
  } else {
    textAreaWidth = view.contentDOM.getBoundingClientRect().width;
  }

  if (Math.abs(_rawMeasureProbeWidth - textAreaWidth) > 1) {
    _rawMeasureProbe.style.width = `${textAreaWidth}px`;
    _rawMeasureProbeWidth = textAreaWidth;
  }

  return _rawMeasureProbe;
}

/**
 * Synchronously measures what the raw (monospace) height of the given document
 * line will be once it transitions from preview widget -> raw text.
 *
 * Must only be called from a user-event handler (mouseup), never from inside a
 * CM6 StateField or ViewPlugin update — the forced layout is intentional there.
 *
 * @param extraClass Optional extra CSS class to apply to the probe before
 *   measuring. Pass "cm-lp-list-line" for list items and "cm-lp-admon-line"
 *   for admonition lines so that their base padding is included — matching
 *   what measureRawLineContentHeightPx returns for those line types.
 */
function probeRawLineHeightPx(
  view: EditorView,
  lineFrom: number,
  extraClass: string | null = null,
): number | null {
  const line = view.state.doc.lineAt(lineFrom);
  const probe = getRawMeasureProbe(view);
  probe.textContent = line.text || "\u200b";
  probe.className = "cm-line" + (extraClass ? ` ${extraClass}` : "");
...
function isPreviewHeightFloorConstrained(lineElement: HTMLElement, measuredHeight: number): boolean {
  if (!lineElement.classList.contains(PREVIEW_HEIGHT_FLOOR_CLASS)) return false;

  const floorHeight = parsePx(lineElement.style.minHeight);
  return floorHeight > 0 && measuredHeight <= floorHeight + MIN_HEIGHT_DELTA_PX;
}
...
function blockTouchesRawSelection(rawLines: Set<number>, blockStart: number, blockEnd: number): boolean {
...
function attachPreviewFocusHandlers(
  ...
  element.addEventListener("mousedown", (e) => {
    ...
    if (!view) return;
    queuePreviewLineFocus(lineFrom);
  });
...
// Freeze decoration rebuilds while the primary pointer is dragging inside the
// editor. This mirrors the desktop stabilization path and avoids mutating live
// preview widgets while Chromium is tracking a native selection gesture.
let pointerDragging = false;
const pendingPointerDragRebuilds = new WeakSet<EditorView>();
...
function queuePreviewLineFocus(lineFrom: number) {
...
function focusPreviewLine(view: EditorView, lineFrom: number) {
  ...
  const trimmedForProbe = line.text.trimStart();
  const probeExtraClass
    = /^(\*{1,5}|\.{1,5})\s+/.test(trimmedForProbe) ? "cm-lp-list-line"
      : /^(NOTE|TIP|WARNING|CAUTION|IMPORTANT|QUESTION):\s+/.test(trimmedForProbe) ? "cm-lp-admon-line"
        : null;
  const probeHeight = probeRawLineHeightPx(view, lineFrom, probeExtraClass);
  const cachedState = view.state.field(livePreviewDecorationField);
  const effects: StateEffect<any>[] = [setEditorFocusedEffect.of(true)];
  if (
    probeHeight !== null
    && Math.abs((cachedState.rawLineHeights.get(lineFrom) ?? 0) - probeHeight) > 1
  ) {
    effects.push(updateHeightCacheEffect.of({
      heights: new Map(),
      rawLineHeights: new Map([[lineFrom, probeHeight]]),
      rawBaseHeight: cachedState.rawBaseHeight,
    }));
  }
...
function openVerseEditorModal(
  ...
  const { overlay, body, footerLeft, footerRight, close } = createBlockEditorModal(view, "Edit Verse Block");
  ...
  citeInput.type = "text";
  citeInput.className = "cm-lp-block-editor-input";
  citeInput.value = citetitle;
  citeInput.spellcheck = false;
  body.appendChild(makeBlockEditorField("Citation", citeInput));

  const contentInput = document.createElement("textarea");
  contentInput.className = "cm-lp-block-editor-textarea";
  contentInput.value = content;
  contentInput.spellcheck = true;
  body.appendChild(makeBlockEditorField("Verse", contentInput));

  const deleteBtn = makeBlockEditorButton("Delete Verse Block");
  deleteBtn.classList.add("cm-lp-block-editor-btn-danger");
  deleteBtn.addEventListener("click", (e) => {
    consumeEvent(e);
    deleteBlockRange(view, blockFrom, blockTo);
  ...
  saveBtn.addEventListener("click", (e) => {
    ...
    view.dispatch({
      changes: {
        from: blockFrom,
        to: blockTo,
        insert: serializeVerse(authorInput.value, citeInput.value, contentInput.value, hadAttributeLine),
      },
    });
    close();
  });
...
async function pickMediaResource(kind: "video" | "audio"): Promise<string> {
  const ipc = await import("../ipc");
  const picked = kind === "video" ? await ipc.openVideoDialog() : await ipc.openAudioDialog();
  if (!picked.filePath) return "";
  const resource = await ipc.createResourceFromFile(picked.filePath);
  if (resource.resourceId && resource.dataUrl) {
    updateResourceUrls([{ id: resource.resourceId, dataUrl: resource.dataUrl }]);
  }
  return resource.resourceId ? `:/${resource.resourceId}` : "";
...
async function openMediaEditorModal(
  view: EditorView,
  kind: "video" | "audio",
  info: VideoBlockInfo | AudioBlockInfo,
) {
  const isVideo = kind === "video";
  const { body, footerLeft, footerRight, close } = createBlockEditorModal(view, isVideo ? "Edit Video" : "Edit Audio");

  const videoState: VideoOptions = isVideo
    ? {
        target: (info as VideoBlockInfo).info.target,
        service: (info as VideoBlockInfo).info.service,
        caption: (info as VideoBlockInfo).info.caption,
        width: (info as VideoBlockInfo).info.width,
        height: (info as VideoBlockInfo).info.height,
        start: (info as VideoBlockInfo).info.start,
        end: (info as VideoBlockInfo).info.end,
        poster: (info as VideoBlockInfo).info.poster,
        theme: (info as VideoBlockInfo).info.theme,
        lang: (info as VideoBlockInfo).info.lang,
        list: (info as VideoBlockInfo).info.list,
        playlist: (info as VideoBlockInfo).info.playlist,
        align: (info as VideoBlockInfo).info.align,
        options: { ...(info as VideoBlockInfo).info.options },
      }
    : ({} as VideoOptions);

  const audioState: AudioOptions = !isVideo
    ? {
        target: (info as AudioBlockInfo).info.target,
        caption: (info as AudioBlockInfo).info.caption,
        start: (info as AudioBlockInfo).info.start,
        end: (info as AudioBlockInfo).info.end,
        options: { ...(info as AudioBlockInfo).info.options },
      }
    : ({} as AudioOptions);

  const makeTextInput = (value: string, onInput: (v: string) => void, placeholder = "") => {
    const el = document.createElement("input");
    el.type = "text";
    el.className = "cm-lp-block-editor-input";
    el.value = value || "";
  ...
  const makeNumberInput = (value: number, onInput: (v: number) => void) => {
    const el = document.createElement("input");
    el.type = "number";
    el.className = "cm-lp-block-editor-input";
  ...
  const makeCheckbox = (checked: boolean, label: string, onChange: (v: boolean) => void) => {
    const wrap = document.createElement("label");
    wrap.className = "cm-lp-block-editor-checkbox";
  ...
  const targetInput = makeTextInput(
    isVideo ? videoState.target : audioState.target,
    (v) => {
      if (isVideo) videoState.target = v;
      else audioState.target = v;
    },
    isVideo ? "video URL, resource, or YouTube/Vimeo ID" : "audio URL or resource",
  );
  body.appendChild(makeBlockEditorField("Target", targetInput));

  const shouldOfferResourcePicker = isVideo
    ? videoState.service === null
    : !/^https?:\/\//i.test(audioState.target);
  if (shouldOfferResourcePicker) {
    const browseBtn = makeBlockEditorButton("Replace from file...", "secondary");
    browseBtn.addEventListener("click", async (e) => {
      consumeEvent(e);
      const target = await pickMediaResource(kind);
      if (!target) return;
      if (isVideo) videoState.target = target;
      else audioState.target = target;
      targetInput.value = target;
    });
    const browseWrap = document.createElement("div");
    browseWrap.className = "cm-lp-block-editor-field";
    browseWrap.appendChild(browseBtn);
    body.appendChild(browseWrap);
  }

  if (isVideo) {
    body.appendChild(makeBlockEditorField("Caption", makeTextInput(videoState.caption, (v) => { videoState.caption = v; })));
    body.appendChild(makeBlockEditorField("Width (px)", makeNumberInput(videoState.width, (v) => { videoState.width = v; })));
    body.appendChild(makeBlockEditorField("Height (px)", makeNumberInput(videoState.height, (v) => { videoState.height = v; })));
    body.appendChild(makeBlockEditorField("Start (s)", makeNumberInput(videoState.start, (v) => { videoState.start = v; })));
    body.appendChild(makeBlockEditorField("End (s)", makeNumberInput(videoState.end, (v) => { videoState.end = v; })));
    if (videoState.service === null) {
      body.appendChild(makeBlockEditorField("Poster", makeTextInput(videoState.poster, (v) => { videoState.poster = v; })));
    }
    if (videoState.service === "youtube") {
      body.appendChild(makeBlockEditorField("Theme", makeTextInput(videoState.theme, (v) => { videoState.theme = v; }, "dark or light")));
      body.appendChild(makeBlockEditorField("Language", makeTextInput(videoState.lang, (v) => { videoState.lang = v; }, "en")));
      body.appendChild(makeBlockEditorField("Playlist ID", makeTextInput(videoState.list, (v) => { videoState.list = v; })));
      body.appendChild(makeBlockEditorField("Playlist Items", makeTextInput(videoState.playlist, (v) => { videoState.playlist = v; })));
    }
    ...
    for (const value of ["", "left", "center", "right"]) {
      ...
      if (videoState.align === value) opt.selected = true;
      alignSelect.appendChild(opt);
    }
    alignSelect.addEventListener("change", () => {
      videoState.align = alignSelect.value as VideoOptions["align"];
    });
    body.appendChild(makeBlockEditorField("Align", alignSelect));

    const optsWrap = document.createElement("div");
    optsWrap.className = "cm-lp-block-editor-checkbox-row";
    optsWrap.appendChild(makeCheckbox(videoState.options.autoplay, "Autoplay", (v) => { videoState.options.autoplay = v; }));
    optsWrap.appendChild(makeCheckbox(videoState.options.loop, "Loop", (v) => { videoState.options.loop = v; }));
    optsWrap.appendChild(makeCheckbox(videoState.options.muted, "Muted", (v) => { videoState.options.muted = v; }));
    optsWrap.appendChild(makeCheckbox(videoState.options.nocontrols, "No controls", (v) => { videoState.options.nocontrols = v; }));
    if (videoState.service === "youtube") {
      optsWrap.appendChild(makeCheckbox(videoState.options.modest, "Modest branding", (v) => { videoState.options.modest = v; }));
      optsWrap.appendChild(makeCheckbox(videoState.options.nofullscreen, "No fullscreen", (v) => { videoState.options.nofullscreen = v; }));
    }
    body.appendChild(optsWrap);
  } else {
    body.appendChild(makeBlockEditorField("Caption", makeTextInput(audioState.caption, (v) => { audioState.caption = v; })));
    body.appendChild(makeBlockEditorField("Start (s)", makeNumberInput(audioState.start, (v) => { audioState.start = v; })));
    body.appendChild(makeBlockEditorField("End (s)", makeNumberInput(audioState.end, (v) => { audioState.end = v; })));

    const optsWrap = document.createElement("div");
    optsWrap.className = "cm-lp-block-editor-checkbox-row";
    optsWrap.appendChild(makeCheckbox(audioState.options.autoplay, "Autoplay", (v) => { audioState.options.autoplay = v; }));
    optsWrap.appendChild(makeCheckbox(audioState.options.loop, "Loop", (v) => { audioState.options.loop = v; }));
    optsWrap.appendChild(makeCheckbox(audioState.options.nocontrols, "No controls", (v) => { audioState.options.nocontrols = v; }));
    body.appendChild(optsWrap);
  }

  const cancelBtn = makeBlockEditorButton("Cancel", "secondary");
  cancelBtn.addEventListener("click", (e) => {
    consumeEvent(e);
    close();
  });
  ...
  const deleteBtn = makeBlockEditorButton("Delete", "secondary");
  deleteBtn.classList.add("cm-lp-block-editor-btn-danger");
  deleteBtn.addEventListener("click", (e) => {
    consumeEvent(e);
    view.dispatch({ changes: { from: info.blockFrom, to: info.blockTo, insert: "" } });
    close();
  });
  footerLeft.appendChild(deleteBtn);
  ...
  saveBtn.addEventListener("click", (e) => {
    consumeEvent(e);
    const block = isVideo ? serializeVideoBlock(videoState) : serializeAudioBlock(audioState);
    if (!block) return;
    view.dispatch({ changes: { from: info.blockFrom, to: info.blockTo, insert: block } });
    close();
  });
...
function headingTopPaddingFraction(level: number): number {
  if (!compactSpacing) {
    return 1 / 1.5;
  }
  if (level === 1) return 0.8 / 1.5;
  if (level === 2) return 0.8 / 1.4;
  return 0.5;
}

function matchedLineDecoration(
  ...
  const extra = targetHeightPx - rawContentHeightPx;
  const paddingTop = extra * topFraction;
  const paddingBottom = extra * (1 - topFraction);
  return Decoration.line({
    class: "cm-lp-stabilized-line",
    attributes: {
      style: [
        `--lp-match-padding-top:${paddingTop}px`,
        `--lp-match-padding-bottom:${paddingBottom}px`,
    ...
    if (stemInnerMatch && idx + 1 < trimmed.length) {
      ...
      if (/^\+{4,}$/.test(nextTrimmed)) {
        ...
        if (stemEnd >= 0) {
          ...
          const mathHtml = `<div class="cm-lp-stemblock" style="padding:0.6em;text-align:center;border-radius:4px">${renderMath(stemLines.join("\n"), resolvedNotation, true)}</div>`;
...
function getBlockStartLineNumber(block: BlockInfo): number {
  ...
  if (block.type === "audio") return block.macroLine;
...
function openPreviewBlockModal(view: EditorView, blockInfo: { block: BlockInfo; blockStart: number; blockEnd: number }): boolean {
  ...
  if (block.type === "verse") {
    let rawVerseContent = "";
    const delimited = /^_{4,}$/.test(view.state.doc.line(block.openLine).text.trim());
    const contentStart = delimited ? block.openLine + 1 : block.openLine;
    const contentEnd = delimited ? block.closeLine - 1 : block.closeLine;
    for (let j = contentStart; j <= contentEnd; j++) {
      if (rawVerseContent) rawVerseContent += "\n";
      rawVerseContent += view.state.doc.line(j).text;
    }
    openVerseEditorModal(view, block.author, block.citetitle, rawVerseContent, blockFrom, blockTo, true);
    return true;
  }
...
function detectBlocks(doc: any): BlockInfo[] {
  ...
  while (i <= doc.lines) {
    ...
    const videoWithTitle = /^\.(?!\.)/.test(text) && i + 1 <= doc.lines
      ? parseVideoMacroLine(doc.line(i + 1).text, text.slice(1))
      : null;
    if (videoWithTitle) {
      blocks.push({ type: "video", titleLine: i, macroLine: i + 1, info: videoWithTitle });
      i += 2;
      continue;
    }

    const standaloneVideo = parseVideoMacroLine(imageLineText);
    if (standaloneVideo) {
      blocks.push({ type: "video", titleLine: -1, macroLine: i, info: standaloneVideo });
      i += 1;
      continue;
    }

    const audioWithTitle = /^\.(?!\.)/.test(text) && i + 1 <= doc.lines
      ? parseAudioMacroLine(doc.line(i + 1).text, text.slice(1))
      : null;
    if (audioWithTitle) {
      blocks.push({ type: "audio", titleLine: i, macroLine: i + 1, info: audioWithTitle });
      i += 2;
      continue;
    }

    const standaloneAudio = parseAudioMacroLine(imageLineText);
    if (standaloneAudio) {
      blocks.push({ type: "audio", titleLine: -1, macroLine: i, info: standaloneAudio });
      i += 1;
      continue;
    }
    ...
    // Detect verse: [verse, author, citetitle] followed by ____ or paragraph.
    // A standalone ____ block remains a regular blockquote, matching Asciidoctor.
    const verseAttrs = parseVerseAttrLine(text);
    if (verseAttrs && i + 1 <= doc.lines) {
      const nextText = doc.line(i + 1).text.trim();
      if (/^_{4,}$/.test(nextText)) {
        ...
        if (closeLine > 0) {
          blocks.push({
            ...
            openLine: i + 1,
            closeLine,
            author: verseAttrs.author,
            citetitle: verseAttrs.citetitle,
          });
          i = closeLine + 1;
          continue;
        }
...
class PreviewLineWidget extends WidgetType {
  ...
  eq(other: PreviewLineWidget): boolean {
    return this.html === other.html
      ...
      && this.cachedHeight === other.cachedHeight;
  }
...
let activeSectionTitles = new Map<string, string>();     // anchor → section title
let activeSectionReftexts = new Map<string, string>();   // anchor → [[id,reftext]] reftext
...
export function resolveXrefDisplayText(
  target: string,
  sectionTitles: Map<string, string>,
  sectionReftexts: Map<string, string>,
): string {
  return (
    sectionReftexts.get(target) ||
    sectionTitles.get(target) ||
    target.replace(/\.(adoc|asciidoc)$/i, "") ||
    target
  );
}
...
type InlinePassthroughKind = "single" | "double" | "triple" | "passMacro";

interface InlinePassthroughPlaceholder {
  kind: InlinePassthroughKind;
  content: string;
}

interface BracketedInlineContent {
  content: string;
  nextIndex: number;
}
...
function isWhitespaceBoundaryChar(char: string | undefined): boolean {
  return char !== undefined && /\s/.test(char);
}

function createPassthroughPlaceholderToken(
  kind: InlinePassthroughKind,
  content: string,
  placeholders: InlinePassthroughPlaceholder[],
  token: string,
): string {
...
function scanBracketedInlineContent(text: string, contentStart: number): BracketedInlineContent | null {
  let depth = 1;
...
function findTriplePlusClose(text: string, startIndex: number): number {
  const closeIndex = text.indexOf("+++", startIndex + 3);
  return closeIndex > startIndex + 3 ? closeIndex : -1;
}

function findDoublePlusClose(text: string, startIndex: number): number {
  const contentStart = startIndex + 2;
  const next = text[contentStart];
  if (next === undefined || next === "+") return -1;

  for (let index = contentStart; index < text.length - 1; index++) {
...
function findSinglePlusClose(text: string, startIndex: number): number {
  const previous = startIndex > 0 ? text[startIndex - 1] : undefined;
  const next = text[startIndex + 1];
  if (
    next === undefined
    || next === "+"
    || previous === "+"
    || isWordBoundaryChar(previous)
    || isWhitespaceBoundaryChar(next)
  ) {
    return -1;
  }
  ...
  for (let index = startIndex + 1; index < text.length; index++) {
    ...
    if (before === "+" || after === "+") continue;
    if (isWhitespaceBoundaryChar(before) || isWordBoundaryChar(after)) continue;
    return index;
  }
  return -1;
}

function extractInlinePassthroughs(
  text: string,
  placeholders: InlinePassthroughPlaceholder[],
  token: string,
): string {
  ...
  for (let index = 0; index < text.length;) {
    if (text.startsWith("pass:[", index)) {
      const bracketed = scanBracketedInlineContent(text, index + "pass:[".length);
      if (bracketed) {
        result += createPassthroughPlaceholderToken("passMacro", bracketed.content, placeholders, token);
    ...
    if (text.startsWith("+++", index)) {
      const closeIndex = findTriplePlusClose(text, index);
      if (closeIndex !== -1) {
        result += createPassthroughPlaceholderToken("triple", text.slice(index + 3, closeIndex), placeholders, token);
        index = closeIndex + 3;
        continue;
      }
    }
    ...
    if (text.startsWith("++", index)) {
      const closeIndex = findDoublePlusClose(text, index);
      if (closeIndex !== -1) {
        result += createPassthroughPlaceholderToken("double", text.slice(index + 2, closeIndex), placeholders, token);
        index = closeIndex + 2;
        continue;
      }
    }
    ...
    if (text[index] === "+") {
      const closeIndex = findSinglePlusClose(text, index);
      if (closeIndex !== -1) {
        result += createPassthroughPlaceholderToken("single", text.slice(index + 1, closeIndex), placeholders, token);
        index = closeIndex + 1;
        continue;
      }
    }
...
export function renderInline(text: string): string {
  ...
  // ── Phase 1b: Extract passthrough forms before any other inline processing ──
  const passthroughPlaceholders: InlinePassthroughPlaceholder[] = [];
  const PASSTHROUGH_TOKEN = "\x00PASS";
  text = extractInlinePassthroughs(text, passthroughPlaceholders, PASSTHROUGH_TOKEN);
  ...
  // Inline block anchors [[id]] or [[id,reftext]] are navigation targets, not visible content.
  result = result.replace(/\[\[([^\[\]]+)\]\]/g, "");

  // Cross-references
  result = result.replace(/xref:([^\[]+)\[([^\]]*)\]/g, (_match, idPart, text) => {
    const sectionAnchor = idPart.includes("#") ? idPart.split("#").slice(1).join("#") : "";
    const nodeId = idPart.split("#")[0];
    const displayText = text || resolveXrefDisplayText(nodeId, activeSectionTitles, activeSectionReftexts);
    let html = `<span class="cm-lp-xref-wrap"><span class="cm-lp-xref" data-node-id="${nodeId}" data-xref-target="${idPart}"${sectionAnchor ? ` data-section-anchor="${sectionAnchor}"` : ""}>${displayText}</span>`;
    if (sectionAnchor) {
      html += `<span class="cm-lp-section-toggle" data-node-id="${nodeId}" data-xref-target="${idPart}" data-section-anchor="${sectionAnchor}" role="button" aria-label="Show linked section" title="Show linked section">${SECTION_TOGGLE_CLOSED}</span>`;
    }
    html += `</span>`;
    return html;
  });
  ...
  result = result.replace(/&lt;&lt;([^&>]+)&gt;&gt;/g, (_m, label) => {
    ...
    const sectionAnchor = label.includes("#") ? label.split("#").slice(1).join("#") : "";
    const displayText = resolveXrefDisplayText(nodeId, activeSectionTitles, activeSectionReftexts);
    let html = `<span class="cm-lp-xref-wrap"><span class="cm-lp-xref" data-node-id="${nodeId}" data-xref-target="${label}"${sectionAnchor ? ` data-section-anchor="${sectionAnchor}"` : ""}>${displayText}</span>`;
    if (sectionAnchor) {
      html += `<span class="cm-lp-section-toggle" data-node-id="${nodeId}" data-xref-target="${label}" data-section-anchor="${sectionAnchor}" role="button" aria-label="Show linked section" title="Show linked section">${SECTION_TOGGLE_CLOSED}</span>`;
    }
    html += `</span>`;
    return html;
  });
  ...
  for (let idx = 0; idx < passthroughPlaceholders.length; idx++) {
    result = result.replace(
      ...
      escapeHtml(passthroughPlaceholders[idx].content),
    );
  }
...
class VersePreviewWidget extends WidgetType {
  ...
  get estimatedHeight(): number {
    return this.cachedHeight > 0 ? this.cachedHeight : 40 + this.lines.length * 24;
  }

  toDOM(): HTMLElement {
    const wrap = document.createElement("div");
    wrap.className = "cm-lp-verseblock-wrap";
    wrap.setAttribute(LINE_HEIGHT_DATA_ATTR, String(this.lineFrom));
    attachBlockModalHandlers(wrap, (view) => {
      openVerseEditorModal(
        view,
        this.author,
        this.citetitle,
        this.rawContent,
    ...
    verse.className = "cm-lp-verseblock";

    const content = document.createElement("pre");
    content.className = "cm-lp-verseblock-content";
    content.innerHTML = this.lines.map((line) => renderInline(line)).join("\n");
    verse.appendChild(content);

    if (this.author) {
      const attr = document.createElement("div");
      attr.className = "cm-lp-verseblock-attribution";
      attr.textContent = "- " + this.author;
      verse.appendChild(attr);
    }
    if (this.citetitle) {
      const cite = document.createElement("div");
      cite.className = "cm-lp-verseblock-citetitle";
      cite.textContent = this.citetitle;
      verse.appendChild(cite);
    }
  ...
  eq(other: VersePreviewWidget): boolean {
    return this.author === other.author
      && this.citetitle === other.citetitle
      && JSON.stringify(this.lines) === JSON.stringify(other.lines)
      && this.rawContent === other.rawContent
      && this.lineFrom === other.lineFrom
      && this.blockFrom === other.blockFrom
...
interface AudioBlockInfo {
  info: ParsedAudioMacro;
  blockFrom: number;
  blockTo: number;
}

function youtubeEmbedSrc(info: ParsedVideoMacro): string {
  const params = new URLSearchParams();
  params.set("rel", "0");
  if (info.options.autoplay) params.set("autoplay", "1");
  if (info.options.muted) params.set("mute", "1");
  if (info.options.loop) {
    params.set("loop", "1");
    params.set("playlist", info.target);
  }
  if (info.options.modest) params.set("modestbranding", "1");
  if (info.options.nocontrols) params.set("controls", "0");
  if (info.options.nofullscreen) params.set("fs", "0");
  if (info.start > 0) params.set("start", String(info.start));
  if (info.end > 0) params.set("end", String(info.end));
  if (info.theme) params.set("theme", info.theme);
  if (info.lang) params.set("cc_lang_pref", info.lang);
  if (info.list) params.set("list", info.list);
  if (info.playlist && !info.options.loop) params.set("playlist", info.playlist);
  return `https://www.youtube.com/embed/${encodeURIComponent(info.target)}?${params.toString()}`;
}

function vimeoEmbedSrc(info: ParsedVideoMacro): string {
  const params = new URLSearchParams();
  if (info.options.autoplay) params.set("autoplay", "1");
  if (info.options.muted) params.set("muted", "1");
  if (info.options.loop) params.set("loop", "1");
  if (info.options.nocontrols) params.set("controls", "0");
  const qs = params.toString();
  return `https://player.vimeo.com/video/${encodeURIComponent(info.target)}${qs ? `?${qs}` : ""}`;
}

function buildMediaFragment(start: number, end: number): string {
  if (start <= 0 && end <= 0) return "";
  const parts: string[] = [];
  parts.push(start > 0 ? String(start) : "");
  if (end > 0) parts.push(String(end));
  return `#t=${parts.join(",")}`;
}

function resolveResourceBackedSrc(target: string): string {
  const resourceMatch = target.match(/^:\/?([a-f0-9]{32})/);
  if (resourceMatch) return resourceUrlCache.get(resourceMatch[1]) || "";
  return "";
}

function resolveLocalMediaSrc(target: string): string {
  const trimmed = target.trim();
  if (!trimmed) return "";
  const resourceSrc = resolveResourceBackedSrc(trimmed);
  if (resourceSrc) return resourceSrc;
  if (/^(https?:|data:|blob:|file:)/i.test(trimmed)) return trimmed;
  if (trimmed.startsWith("/") || /^[a-zA-Z]:[\\/]/.test(trimmed)) return `file://${trimmed}`;
  return trimmed;
}

class VideoPreviewWidget extends WidgetType {
  constructor(
    readonly info: ParsedVideoMacro,
    readonly lineFrom: number,
    readonly blockFrom: number,
    readonly blockTo: number,
    readonly cachedHeight: number = -1,
  ...
  get estimatedHeight(): number { return this.cachedHeight > 0 ? this.cachedHeight : 220; }

  toDOM(): HTMLElement {
    const wrap = document.createElement("div");
    wrap.className = "cm-lp-video-block";
    wrap.setAttribute(LINE_HEIGHT_DATA_ATTR, String(this.lineFrom));
    wrap.style.display = "flex";
    wrap.style.flexDirection = "column";
    wrap.style.alignItems = this.info.align === "left" ? "flex-start" : this.info.align === "right" ? "flex-end" : "center";
    wrap.style.margin = "var(--lp-image-mb) 0";
    attachBlockModalHandlers(wrap, (view) => {
      openVideoEditorModal(view, { info: this.info, blockFrom: this.blockFrom, blockTo: this.blockTo });
    });

    if (this.info.caption.trim()) {
      const title = document.createElement("div");
      title.className = "cm-lp-image-block-title";
      title.style.textAlign = "center";
      title.innerHTML = renderInline(this.info.caption.trim());
      wrap.appendChild(title);
    }

    const width = this.info.width > 0 ? this.info.width : 640;
    const height = this.info.height > 0 ? this.info.height : 360;

    if (this.info.service === "youtube" || this.info.service === "vimeo") {
      const iframe = document.createElement("iframe");
      iframe.className = "cm-lp-video-iframe";
      iframe.src = this.info.service === "youtube" ? youtubeEmbedSrc(this.info) : vimeoEmbedSrc(this.info);
      iframe.width = String(width);
      iframe.height = String(height);
      iframe.style.maxWidth = "100%";
      iframe.allow = "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture";
      iframe.referrerPolicy = "strict-origin-when-cross-origin";
      if (!this.info.options.nofullscreen) iframe.setAttribute("allowfullscreen", "");
      iframe.setAttribute("frameborder", "0");
      wrap.appendChild(iframe);
      return wrap;
    }

    const src = resolveLocalMediaSrc(this.info.target);
    if (!src) {
      const placeholder = document.createElement("div");
      placeholder.className = "cm-lp-video-loading";
    ...
    video.controls = !this.info.options.nocontrols;
    video.autoplay = this.info.options.autoplay;
    video.loop = this.info.options.loop;
    video.muted = this.info.options.muted;
    video.preload = "metadata";
    if (this.info.width > 0) video.width = width;
    if (this.info.height > 0) video.height = height;
    if (this.info.poster) video.poster = resolveLocalMediaSrc(this.info.poster) || this.info.poster;
    video.src = `${src}${buildMediaFragment(this.info.start, this.info.end)}`;
    video.style.maxWidth = "100%";
    wrap.appendChild(video);
    return wrap;
  }

  eq(other: VideoPreviewWidget): boolean {
    return JSON.stringify(this.info) === JSON.stringify(other.info)
      && this.lineFrom === other.lineFrom
      && this.blockFrom === other.blockFrom
      && this.blockTo === other.blockTo;
  }
...
class AudioPreviewWidget extends WidgetType {
  constructor(
    readonly info: ParsedAudioMacro,
    readonly lineFrom: number,
    readonly blockFrom: number,
    readonly blockTo: number,
    readonly cachedHeight: number = -1,
  ...
  toDOM(): HTMLElement {
    const wrap = document.createElement("div");
    wrap.className = "cm-lp-audio-block";
    wrap.setAttribute(LINE_HEIGHT_DATA_ATTR, String(this.lineFrom));
    wrap.style.display = "flex";
    wrap.style.flexDirection = "column";
    ...
    wrap.style.margin = "var(--lp-image-mb) 0";
    attachBlockModalHandlers(wrap, (view) => {
      openAudioEditorModal(view, { info: this.info, blockFrom: this.blockFrom, blockTo: this.blockTo });
    });
    ...
    if (!src) {
      const placeholder = document.createElement("div");
      placeholder.className = "cm-lp-audio-loading";
    ...
    audio.controls = !this.info.options.nocontrols;
    audio.autoplay = this.info.options.autoplay;
    audio.loop = this.info.options.loop;
    audio.preload = "metadata";
    audio.src = `${src}${buildMediaFragment(this.info.start, this.info.end)}`;
    audio.style.width = "min(100%, 480px)";
    wrap.appendChild(audio);
    return wrap;
  }

  eq(other: AudioPreviewWidget): boolean {
    return JSON.stringify(this.info) === JSON.stringify(other.info)
      && this.lineFrom === other.lineFrom
      && this.blockFrom === other.blockFrom
      && this.blockTo === other.blockTo;
...
function openVideoEditorModal(view: EditorView, info: VideoBlockInfo) {
  void openMediaEditorModal(view, "video", info);
}

function openAudioEditorModal(view: EditorView, info: AudioBlockInfo) {
  void openMediaEditorModal(view, "audio", info);
}
...
function buildDecorations(
  ...
  const doc = state.doc;
  const cursorLine = doc.lineAt(state.selection.main.head).number;
  const hasRangeSelection = state.selection.ranges.some((range) => !range.empty);
  ...
  while (i <= doc.lines) {
    ...
    if (block && i === blockStart) {
      ...
      const selectionTouchesBlock = editorHasFocus && blockTouchesRawSelection(rawLines, blockStart, blockEnd);
      ...
      if (block.type === "docheader") {
        ...
        if (!cursorInBlock) {
          ...
          builder.add(firstLine.from, firstLine.from, specialBlockLineDecoration);
          builder.add(firstLine.from, firstLine.to, Decoration.replace({
            widget: new VideoPreviewWidget(block.info, firstLine.from, fromPos, toPos, heightData.lineHeights.get(firstLine.from) ?? -1),
          }));
          for (let j = blockStart + 1; j <= blockEnd; j++) {
            const line = doc.line(j);
            builder.add(line.from, line.from, hiddenLineDecoration);
        ...
        if (!cursorInBlock) {
          ...
          builder.add(firstLine.from, firstLine.from, specialBlockLineDecoration);
          builder.add(firstLine.from, firstLine.to, Decoration.replace({
            widget: new AudioPreviewWidget(block.info, firstLine.from, fromPos, toPos, heightData.lineHeights.get(firstLine.from) ?? -1),
          }));
          for (let j = blockStart + 1; j <= blockEnd; j++) {
            const line = doc.line(j);
            builder.add(line.from, line.from, hiddenLineDecoration);
          ...
          for (let j = blockStart; j <= blockEnd; j++) {
            ...
            if (j === cursorLine) {
              ...
              builder.add(line.from, line.from, padStyle
                ...
                : codeDelimDecoration);
              if (overlayEditingMode && !hasRangeSelection) {
        ...
        if (cursorInBlock) {
          ...
          for (let j = contentStart; j <= contentEnd; j++) {
            const rawLineText = doc.line(j).text;
            if (rawVerseContent) rawVerseContent += "\n";
            rawVerseContent += rawLineText;
            contentLines.push(rawLineText);
          }
          const firstLine = doc.line(blockStart);
          builder.add(firstLine.from, firstLine.from, specialBlockLineDecoration);
          builder.add(firstLine.from, firstLine.to, Decoration.replace({
            widget: new VersePreviewWidget(
              contentLines,
              block.author,
              block.citetitle,
              rawVerseContent,
    ...
    if (editorHasFocus && rawLines.has(i)) {
      if (headingMatch) {
        ...
        const renderedHeight = cachedHeight
          ?? estimateHeadingLineHeightPx(rawBaseHeightPx, level);
        const rawContentHeight = heightData.rawLineHeights.get(line.from) ?? rawBaseHeightPx;
        if (renderedHeight) {
          const decoration = matchedLineDecoration(
            renderedHeight,
            rawContentHeight,
            headingTopPaddingFraction(level),
          );
          if (decoration) {
        ...
        if (cachedHeight != null) {
          const rawContentHeight = heightData.rawLineHeights.get(line.from)
            ?? estimateListLineHeightPx(rawBaseHeightPx);
          const decoration = matchedLineDecoration(cachedHeight, rawContentHeight);
          if (decoration) {
            builder.add(line.from, line.from, decoration);
          }
        }
    ...
    if (i === docTitleLine && authorBylineHtml && headingMatch && headingMatch[1].length === 1) {
      html = html
    ...
    // Stabilize preview height without feeding the floored value back into the
    // intrinsic preview-height cache measured in scheduleHeightMeasurement().
    const knownRawH = heightData.rawLineHeights.get(line.from);
    const previewHeightFloor = knownRawH != null && knownRawH > intrinsicWidgetHeight + MIN_HEIGHT_DELTA_PX
      ? knownRawH
      : null;
    const widgetHeight = previewHeightFloor ?? intrinsicWidgetHeight;
    if (previewHeightFloor != null) {
      builder.add(line.from, line.from, Decoration.line({
        class: PREVIEW_HEIGHT_FLOOR_CLASS,
        attributes: {
          style: [
            `min-height:${previewHeightFloor}px`,
            "box-sizing:border-box",
          ].join(";"),
        },
      }));
  ...
  class {
    ...
    constructor(view: EditorView) {
      ...
      this.pointerUpHandler = (event: PointerEvent) => {
        ...
        if (!pendingPointerDragRebuilds.has(this.boundView)) {
          pendingPointerDragRebuilds.add(this.boundView);
          queueMicrotask(() => {
            pendingPointerDragRebuilds.delete(this.boundView);
            try {
              this.boundView.dispatch({ effects: refreshLivePreviewEffect.of(null) });
            } catch {
              // The Joplin webview/editor may have been torn down.
            }
          });
        }
      };
      this.pointerCancelHandler = this.pointerUpHandler;
      view.dom.addEventListener("pointerdown", this.pointerDownHandler, true);
    ...
    destroy() {
      this.boundView.dom.removeEventListener("pointerdown", this.pointerDownHandler, true);
      ...
      if (pointerDragging) pointerDragging = false;
      if (_activeView === this.boundView) {
        _activeView = null;
      }
    }
      ...
      view.requestMeasure({
        read(view) {
          ...
          for (const element of view.dom.querySelectorAll<HTMLElement>(`[${LINE_HEIGHT_DATA_ATTR}]`)) {
            ...
            const from = Number.parseInt(rawFrom, 10);
            if (!Number.isFinite(from)) continue;
            const renderedLine = element.closest<HTMLElement>(".cm-line");
            const measurementTarget = renderedLine instanceof HTMLElement ? renderedLine : element;
            const height = measureElementHeightPx(measurementTarget);
            if (height > 0 && !(renderedLine && isPreviewHeightFloorConstrained(renderedLine, height))) {
              lineHeights.set(from, Math.max(height, lineHeights.get(from) ?? 0));
            }
          }
          const { rawLineNumbers: rawLines } = view.state.field(livePreviewDecorationField);
        ...
        write(measured, view) {
          ...
          if (!changed) {
            for (const [k, v] of measured.lineHeights) {
              if (Math.abs((currentState.lineHeights.get(k) ?? 0) - v) > 2) {
                changed = true;
                break;
              }
            }
          ...
          if (!changed) {
            for (const [k, v] of measured.rawLineHeights) {
              if (Math.abs((currentState.rawLineHeights.get(k) ?? 0) - v) > 2) {
                changed = true;
                break;
              }
            }
...
const livePreviewTheme = EditorView.theme({
  ...
  ".cm-line.cm-lp-preview-height-floor": {
    boxSizing: "border-box",
  },
  ".cm-gutters": {
    fontFamily: "monospace",
  ...
  // Verse block — preserves whitespace via <pre> + white-space: pre-wrap
  ".cm-lp-verseblock-wrap": {
    padding: "0",
  },
  ".cm-lp-verseblock": {
    margin: "var(--lp-quote-margin)",
  ...
  ".cm-lp-video-loading, .cm-lp-audio-loading": {
    color: "var(--asciidoc-placeholder, #777)",
    fontSize: "0.95em",
    padding: "0.6em 0",
  },
  ".cm-lp-video, .cm-lp-video-iframe": {
    borderRadius: "6px",
    background: "#000",
  },
  ".cm-lp-audio": {
  ...
  // Hover shading — controlled by --lp-block-shading-hover custom property (set by panel.ts)
  // Uses calc() to multiply alpha by 0 or 1 based on the toggle.
  ".cm-line.cm-lp-special-block-line:hover": {
    background: "rgba(0,0,0, calc(0.06 * var(--lp-block-shading-hover, 0)))",
    boxShadow: "inset 0 0 0 calc(1px * var(--lp-block-shading-hover, 0)) rgba(128,128,128,0.18)",
  },
  ".cm-lp-stemblock:hover": {
    background: "rgba(0,0,0, calc(0.06 * var(--lp-block-shading-hover, 0)))",
    borderColor: "rgba(128,128,128, calc(0.18 * var(--lp-block-shading-hover, 0)))",
  },
  ".cm-lp-mermaid-block:hover": {
    background: "rgba(0,0,0, calc(0.06 * var(--lp-block-shading-hover, 0)))",
    borderColor: "rgba(128,128,128, calc(0.18 * var(--lp-block-shading-hover, 0)))",
  },
  ...
  mousedown(event, view) {
    ...
    if (event.button === 0 && !isInteractivePreviewTarget(target) && !isFloatingPreviewTarget(target)) {
      ...
      if (candidateLineFrom != null) {
        consumeEvent(event);
        queuePreviewLineFocus(candidateLineFrom);
        return true;
      }
    }
