...
import { searchNotes, getNoteSections, resolveXrefTarget } from "../ipc";
...
async function handleNoteCompletion(
  ...
  const noteOptions: Completion[] = results.map((note) => ({
    ...
    apply: (view: any, _completion: any, _from: number, to: number) => {
      const displayTitle = note.title.replace(/[<>\r\n]+/g, " ").trim() || note.id;
      const text = `<<${note.id},${displayTitle}>>`;
      view.dispatch({
...
async function handleSectionCompletion(
  ...
  try {
    const resolved = await resolveXrefTarget(getCurrentNoteId(), nodeId);
    if (!resolved) return null;
    resolvedNoteId = resolved.noteId;
  } catch {
    return null;
  }
  ...
  try {
    const response = await getNoteSections(resolvedNoteId);
