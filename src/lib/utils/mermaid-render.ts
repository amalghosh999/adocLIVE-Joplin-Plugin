...
let currentThemeConfig: ThemeMermaidConfig = { theme: "default" };
...
export function setMermaidTheme(isDark: boolean): void {
  setMermaidThemeConfig({ theme: isDark ? "dark" : "default" });
}

export function setMermaidThemeConfig(config: ThemeMermaidConfig): void {
  const nextConfig = config || { theme: "default" };
  if (JSON.stringify(nextConfig) === JSON.stringify(currentThemeConfig) && initialized) return;
...
function initMermaidConfig() {
  ...
  if (currentThemeConfig.theme === "dark") {
  ...
  if (currentThemeConfig.variables) {
    config.themeVariables = {
      ...(config.themeVariables || {}),
