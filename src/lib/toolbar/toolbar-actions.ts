...
import { type VideoOptions, serializeVideoBlock } from "../utils/video-macro";
import { type AudioOptions, serializeAudioBlock } from "../utils/audio-macro";
...
export function handleVideo(options: VideoOptions) {
  const block = serializeVideoBlock(options);
  if (!block) return;
  insertText(`\n${block}\n`);
}

export function handleAudio(options: AudioOptions) {
  const block = serializeAudioBlock(options);
  if (!block) return;
  insertText(`\n${block}\n`);
}
...
export const mediaItems: ToolbarItem[] = [
  ...
  { id: "video", label: "Video", title: "Video", icon: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="14" height="12" rx="2"/><polygon points="16 10 22 6 22 18 16 14" fill="currentColor"/></svg>`, action: () => {}, hasSplit: true, splitId: "video" },
  { id: "audio", label: "Audio", title: "Audio", icon: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="3 9 8 9 13 5 13 19 8 15 3 15"/><path d="M16 9a4 4 0 0 1 0 6"/><path d="M19 7a7 7 0 0 1 0 10"/></svg>`, action: () => {}, hasSplit: true, splitId: "audio" },
